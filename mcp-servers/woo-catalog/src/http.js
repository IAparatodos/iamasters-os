...
//   GET  /health  → healthcheck simple (para uptime/monitor del hosting)
//
...
const httpServer = createServer(async (req, res) => {
  ...
  // Stateless: una instancia de server+transport por petición.
  if (req.method === "POST") {
    let body;
    try {
  ...
  // GET/DELETE en /mcp no aplican en modo stateless.
...
httpServer.listen(PORT, () => {
  console.error(
    `[woo-catalog-mcp] ${config.brandName} listo (HTTP, read-only) en :${PORT}${MCP_PATH} sobre ${config.baseUrl}`
  );
});
